  ...
  imap(0)
{
  ...
  speech=nh_public.advertise<std_msgs::String>("/speech",5);
  ...
  nh_private.param("safe_execution", ivSafeExecution, false);
...
FootstepNavigation::plan()
{
  ...
  if (!updateStart())
  {
    ROS_ERROR("Start pose not accessible!, In Plan()");
    return false;
  }
...
FootstepNavigation::replan()
{
  ...
  {
#if 1
    ROS_ERROR("Start pose not accessible!, In RePlan()");
    return false;
#endif
#if 0
    ROS_ERROR("Start pose not accessible!, In RePlan()");
...
FootstepNavigation::executeFootsteps()
{
  ...
  ROS_INFO("Start walking towards the goal.executeFootstep");
  ...
  ivPlanner.clearFootstepPathVis(0);
  std_msgs::String msg;
  msg.data="I arrived at the goal";
  speech.publish(msg);
  // free the lock
  ivExecutingFootsteps = false;
}
...
FootstepNavigation::activeCallback()
{
	...
	ROS_INFO("Start walking towards the goal.activeCallback");
}
		...
		const humanoid_nav_msgs::ExecFootstepsFeedbackConstPtr& fb)
{
	...
	if (executed_steps_idx >= ivControlStepIdx + 2)
	{
		...
		ROS_ERROR("Footstep execution incorrect.");
	...
	{
		ROS_ERROR("planned (%f, %f, %f, %i) vs. executed (%f, %f, %f, %i)",
		...
		const nav_msgs::OccupancyGridConstPtr& occupancy_map)
{
	...
	*/	
	...
	//	gridmap_2d::GridMap2DPtr map(new gridmap_2d::GridMap2D(occupancy_map));
	boost::mutex::scoped_lock lock(io_mutex);
	map=boost::make_shared<gridmap_2d::GridMap2D>(occupancy_map);
	ivPlanner.updateMap(map);
	if(checkblock(map))
	{

		std_msgs::String msg;
		msg.data="detect the obstacle!";
		speech.publish(msg);
		ROS_ERROR("------block------");
		if (ivExecutingFootsteps)
		{
...
bool FootstepNavigation::checkblock(gridmap_2d::GridMap2DPtr map)
{
	if(ivPlanner.getPathSize()<=1)
		return false;
	int box=0;
	//	gridmap_2d::GridMap2DPtr map=ivPlanner.getMap();
	float res=map->getResolution();
	for(state_iter_t it=ivPlanner.getPathBegin();it!=ivPlanner.getPathEnd();it++)
	{
		float x=it->getX();
		float y=it->getY();
		float startx=x-box*res;
		for(int i=-box;i<=box;i++)
		{
			float starty=y-box*res;
			for(int j=-box;j<=box;j++)
			{
				if(map->isOccupiedAt(startx,starty))
					return true;
				starty+=res;
			}
			startx+=res;
