  ...
  ivMarkerNamespace("")
{
  ...
  speech=nh_public.advertise<std_msgs::String>("/speech",1);	
...
FootstepPlanner::run()
{
  ...
  else
  {
    ...
    broadcastRandomNodesVis();
    std_msgs::String msg;
    msg.data="i can't find a solution";
    speech.publish(msg);
...
FootstepPlanner::setGoal(float x, float y, float theta)
{
      ...
      ivPlannerEnvironmentPtr->occupied(foot_right))
  {
    ...
    std_msgs::String msg;
    msg.data="the goal you set isn't correct";
    speech.publish(msg);
    ivGoalPoseSetUp = false;
    return false;
  }
...
//  ROS_ERROR("scale.x = %lf", ivEnvironmentParams.footsize_x);
 // ROS_ERROR("scale.y = %lf",ivEnvironmentParams.footsize_y);
...
/*
void
FootStepPlanner::clearfootstep()
{
	nav_msgs::Path path_msg;
	path_msg.header.stamp=ros::Time::now();
	path_msg.header.frame_id=ivMapPtr->getFrameID();
	ivPathVisPub.publish(path_msg);
